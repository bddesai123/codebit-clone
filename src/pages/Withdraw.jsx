...
const Withdraw = () => {
  ...
  const { walletAddress: contextWalletAddress, provider } = useWallet();

  const handleWithdraw = async () => {
    try {
      // Check if wallet is connected
      if (!provider || !contextWalletAddress) {
        console.error("Wallet not connected");
        return;
      }

      const contractAddress = import.meta.env.VITE_STAKING_CONTRACT; // Replace with your actual contract address
      const contractABI = [
        {
          "type": "constructor",
          "name": "",
      ...
      // Convert withdrawAmount to ethers.BigNumber
      const withdrawAmountBN = ethers.utils.parseEther(withdrawAmount);

      // Call your contract's withdraw function
      const tx = await contract.withdraw(withdrawAmountBN);
      ...
      const response = await fetch(`${import.meta.env.VITE_WITHDRAW_API}`,{
        method: "POST",
        headers: {
      ...
      if (data.message === "Withdrawal successful") {
        toast.success(`walletBalance: ${data.virtualMoney}`);
      }else{
